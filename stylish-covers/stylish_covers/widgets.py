...
from .generator import artwork_for, merged_settings, render_cover
    ...
    *entries* is a list of dicts with the keys "book_id", "info" (BookInfo)
    ...
    def _build_ui(self):
        ...
        self.original_check = QCheckBox(
            'Reuse the illustration kept from the last generation', self)
        self.original_check.setToolTip(
            'When you picked an illustration with Change image..., it is kept '
            'so you can retune the preset later without picking it again.')
        self.original_check.stateChanged.connect(self._schedule)
    ...
    def _load_settings_into_ui(self):
        ...
        self.original_check.setChecked(bool(s.get('reuse_saved_artwork', True)))
    ...
    def collect_settings(self):
        ...
        s['reuse_saved_artwork'] = self.original_check.isChecked()
    ...
    def _refresh_artwork_label(self):
        entry = self._current()
        custom = entry.get('custom_image')
        if custom:
            self.image_label.setText(os.path.basename(custom))
        elif self.original_check.isChecked() and entry.get('artwork'):
            self.image_label.setText('Illustration kept from the last '
                                     'generation')
        elif entry.get('image'):
            self.image_label.setText(
                'The book cover itself. If it already shows a title, it will '
                'be printed a second time - use Change image... instead.')
        else:
            self.image_label.setText('No cover - a gradient will be used')
        self.original_check.setEnabled(bool(entry.get('artwork')))
